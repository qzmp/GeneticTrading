...
\begin{figure}[h]
\centering
\caption{Szeregi czasowe wykorzystywane przy testach}
\label{data}
\begin{tikzpicture}
\begin{groupplot}[ group style={group size= 2 by 3, horizontal sep=3cm,  vertical sep=3cm}, ylabel near ticks, xlabel near ticks, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=5}, width=.45\textwidth, no markers]

\nextgroupplot[align=center, title={\scriptsize 1.1.2016 00:00:00 - 15.1.2016 00:00:00}, ylabel=cena zamknięcia, xlabel=liczba okresów]
\addplot 
table [col sep = semicolon,x index = 0, y index=4] {data1.csv};

\nextgroupplot[align=center, title={\scriptsize 15.1.2016 00:00:00 - 1.2.2016 00:00:00},ylabel=cena zamknięcia, xlabel=liczba okresów]
\addplot 
table [col sep = semicolon,x index = 0, y index=4] {data2.csv};

\nextgroupplot[align=center, title={\scriptsize 1.2.2016 00:00:00 - 15.2.2016 00:00:00}, ylabel=cena zamknięcia, xlabel=liczba okresów]
\addplot 
table [col sep = semicolon,x index = 0, y index=4] {data3.csv};

\nextgroupplot[align=center, title={\scriptsize 15.2.2016 00:00:00 - 1.3.2016 00:00:00}, ylabel=cena zamknięcia, xlabel=liczba okresów]
\addplot 
table [col sep = semicolon,x index = 0, y index=4] {data4.csv};

\nextgroupplot[align=center, title={\scriptsize 1.3.2016 00:00:00 - 15.3.2016 00:00:00}, ylabel=cena zamknięcia, xlabel=liczba okresów]
\addplot 
table [col sep = semicolon,x index = 0, y index=4] {data5.csv};

\nextgroupplot[align=center, title={\scriptsize 15.3.2016 00:00:00 - 1.4.2016 00:00:00}, ylabel=cena zamknięcia, xlabel=liczba okresów]
\addplot 
table [col sep = semicolon,x index = 0, y index=4] {data6.csv};
...
Początkowe wyniki, z parametrami wybranymi intuicyjnie, są porównywalne do tych z literatury. Utworzone strategie dobrze dopasowują się do zbioru treningowego, otrzymując stały, dodatni wynik. Na zbiorze testowym ujawnia się jednak bardzo duże zróżnicowanie strategii. Wiele z nich jest stratnych, jednak średni zwrot jest pozytywny.
...
Na wykresach \ref{wykresWysokosc3} można też zauważyć, że podczas gdy zastosowane ograniczenie wysokości drzewa dobrze ogranicza jego wartość maksymalną, to średnia wysokość może mieć tendencję do spadku. Oczywiście takie było założenie, zgodnie z literaturą mniejsze drzewa mają większe możliwości generalizacji. Spadek wysokości drzewa jest więc teoretycznie pozytywnym zjawiskiem, jednak nie występuje on zawsze - w niektórych przebiegach algorytmu wysokość drzewa utrzymuje się blisko wartości maksymalnej \ref{wykresWysokosc4}.

\begin{figure}[h]
\caption {Zależność pomiędzy średnią oceną osobników a średnią ich wysokością}
\label{wykresWysokosc3}
\begin{tikzpicture}
\begin{groupplot}[group style={group size= 1 by 2, horizontal sep=3cm,  vertical sep=3cm}, ylabel near ticks, xlabel near ticks, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=5}, width=.8\textwidth]
\nextgroupplot[title=Średnia ocena osobników wraz z liczbą generacji, ylabel=średni zysk na zbiorze treningowym, xlabel=numer generacji]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {treeSize3.csv};

\nextgroupplot[title=Średnia wysokość drzewa wraz z liczbą generacji, ylabel=średnia wysokość drzewa, xlabel=numer generacji]
\addplot 
table [col sep = semicolon, x index=0, y index=4] {treeSize3.csv};
\end{groupplot}
\end{tikzpicture}
\end{figure}

\begin{figure}[h]
\caption{Średnia wysokość drzewa utrzymująca się w pobliżu maksymalnej = 5}
\centering
\label{wykresWysokosc4}
\begin{tikzpicture}
\begin{axis}[ylabel near ticks, xlabel near ticks,  ylabel=średnia wysokość drzewa strategii, xlabel=numer generacji]
\addplot 
table [col sep = semicolon, x index=0, y index=4] {treeSize4.csv};
\end{axis}
\end{tikzpicture}
\end{figure}

\begin{figure}[h]
\caption{Średnia ocena osobników na danych treningowych z różnym parametrem wysokości drzewa}
\centering
\label{wykresWysokoscSummary1}
\begin{tikzpicture}
\begin{axis}[ylabel near ticks, xlabel near ticks,  ylabel=średnia ocena strategii, xlabel=ustalona wysokość drzewa]
\addplot
 [error bars/.cd, y dir = both, y explicit]
table [col sep = semicolon, x index=0, y index=1,  y error index=2] {treeSizeSummaryTrain.csv};
\end{axis}
\end{tikzpicture}
\end{figure}
...
\begin{axis}[ylabel near ticks, xlabel near ticks,  ylabel=średnia ocena strategii, xlabel=ustalona wysokość drzewa]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {treeSizeSummaryTest.csv};
\end{axis}
\end{tikzpicture}
\end{figure}
...
\begin{axis}[ylabel near ticks, xlabel near ticks, ylabel=zysk na zbiorze treningowym, xlabel=szansa na krzyżowanie]
\addplot 
plot [error bars/.cd, y dir = both, y explicit]
table [col sep = semicolon, x index=0, y index=1, y error index=2] {crossingResults1.csv};
\end{axis}
\end{tikzpicture}
\end{figure}
...
\begin{axis}[ylabel near ticks, xlabel near ticks, ylabel=zysk na zbiorze testowym, xlabel=szansa na krzyżowanie]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {crossingResultsTest.csv};
\end{axis}
\end{tikzpicture}
\end{figure}
...
\begin{figure}[h]
\centering
\caption{Działanie algorytmu z różnymi parametrami mutacji na danych treningowych}
\label{wykresyMutacja}
\begin{tikzpicture}
\begin{groupplot}[ group style={group size= 2 by 2, horizontal sep=3cm,  vertical sep=3cm}, ylabel near ticks, xlabel near ticks, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=5}, width=.45\textwidth]

\nextgroupplot[align=center, title={\scriptsize Działanie algorytmu z różnymi \\ \scriptsize szansami na mutację węzła wewnętrznego}, ylabel=zysk na zbiorze treningowym, xlabel=szansa na mutację]
\addplot 
plot [error bars/.cd, y dir = both, y explicit]
table [col sep = semicolon, x index=0, y index=1, y error index=2] {mutation1Results.csv};
 
\nextgroupplot[align=center, title={\scriptsize Działanie algorytmu z różnymi \\ \scriptsize szansami na mutację liścia}, ylabel=zysk na zbiorze treningowym, xlabel=szansa na mutację]
\addplot 
plot [error bars/.cd, y dir = both, y explicit]
table [col sep = semicolon, x index=0, y index=1, y error index=2] {mutation4Results.csv};

\nextgroupplot[align=center, title={\scriptsize Działanie algorytmu z różnymi \\ \scriptsize szansami na obcięcie drzewa}, ylabel=zysk na zbiorze treningowym, xlabel=szansa na mutację]
\addplot 
plot [error bars/.cd, y dir = both, y explicit]
table [col sep = semicolon, x index=0, y index=1, y error index=2] {mutation2Results.csv};

\nextgroupplot[align=center, title={\scriptsize Działanie algorytmu z różnymi \\ \scriptsize szansami na wzrost drzewa}, ylabel=zysk na zbiorze treningowym, xlabel=szansa na mutację]
\addplot 
plot [error bars/.cd, y dir = both, y explicit]
table [col sep = semicolon, x index=0, y index=1, y error index=2] {mutation3Results.csv};
\end{groupplot}
\end{tikzpicture}
\end{figure}

Na wykresach \ref{wykresyMutacja} widoczna jest nieregularność. Wynika ona z faktu, że nawet wielokrotne powtórzone badania cechują się dużą zmiennością wyników. Widoczna jest jednak przydatność operatorów w dopasowywaniu osobników do danych treningowych. Na wykresach prezentujących wpływ mutacji liścia czy węzła wewnętrznego otrzymywane wyniki są ewidentnie niższe gdy dany operator nie jest używany. W przypadku kolejnych dwóch operatorów mutacji otrzymywane wyniki są niepewne. Operatory te mają jednak rolę w kontroli wysokości drzewa strategii. Widoczne to jest na wykresach prezentujących średnią wysokość drzewa w kolejnych generacjach \ref{wykresyWysokosc1}.
...
\begin{tikzpicture}
\begin{groupplot}[group style={group size= 1 by 2, horizontal sep=3cm,  vertical sep=3cm}, ylabel near ticks, xlabel near ticks, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=5}, width=.8\textwidth]
\nextgroupplot[title=Działanie algorytmu bez operatora skracania drzewa dla podanej wysokości = 5, ylabel near ticks, xlabel near ticks,  ylabel=średnia wysokość drzewa strategii, xlabel=numer generacji]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {treeSize1.csv};

\nextgroupplot[title=Działanie algorytmu bez operatora zwiększania drzewa dla podanej wysokości = 5, ylabel near ticks, xlabel near ticks,  ylabel=średnia wysokość drzewa strategii, xlabel=numer generacji]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {treeSize2.csv};

\end{groupplot}
\end{tikzpicture}
\end{figure}

\subsection{Mutacja na danych testowych}

Analizując dane testowe trudno ocenić dziłanie różnych operatorów mutacji. Powtarza się trend małych zmian w zysku i ciągłych duzych odchyleń standardowych. Odchylenia nie zostały przedstawione na wykresach, bo uczyniłyby je nieczytelnymi. Wynoszą około 2000 pipsów, podczas gdy średnie wartości jedynie około 200.

\begin{figure}[h]
\centering
\caption{Działanie algorytmu z różnymi parametrami mutacji na danych testowych}
\label{wykresyMutacja}
\begin{tikzpicture}
\begin{groupplot}[ group style={group size= 2 by 2, horizontal sep=3cm,  vertical sep=3cm}, ylabel near ticks, xlabel near ticks, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=5}, width=.45\textwidth]

\nextgroupplot[align=center, title={\scriptsize Działanie algorytmu z różnymi \\ \scriptsize szansami na mutację węzła wewnętrznego}, ylabel=zysk na zbiorze testowym, xlabel=szansa na mutację]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {mutation1Results - test.csv};
 
\nextgroupplot[align=center, title={\scriptsize Działanie algorytmu z różnymi \\ \scriptsize szansami na mutację liścia}, ylabel=zysk na zbiorze testowym, xlabel=szansa na mutację]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {mutation4Results - test.csv};

\nextgroupplot[align=center, title={\scriptsize Działanie algorytmu z różnymi \\ \scriptsize szansami na obcięcie drzewa}, ylabel=zysk na zbiorze testowym, xlabel=szansa na mutację]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {mutation2Results - test.csv};

\nextgroupplot[align=center, title={\scriptsize Działanie algorytmu z różnymi \\ \scriptsize szansami na wzrost drzewa}, ylabel=zysk na zbiorze testowym, xlabel=szansa na mutację]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {mutation3Results - test.csv};
\end{groupplot}
\end{tikzpicture}
\end{figure}


\section {Liczba generacji i liczba osobników w populacji}

Analizując wykresy przedstawiające działanie algorytmu z różnymi wielkościami populacji czy liczbą generacji \ref{wykresyWielkosc1}, nasuwa się stwierdzenie, że aby osiągnąć najlepszy wynik na zbiorze testowym, powinno się ograniczać wyniki na zbiorze treningowym. Oba parametry maja podobne działanie - umożliwiają na eksploracje większej przestrzeni rozwiązań, zwiększając  potencjalną liczbę osobników w trakcje ewolucji. Ten zysk jest jednak obarczony kosztem - zwiększanie populacji lub liczby generacji liniowo zwiększa złożoność obliczeniową algorytmu. Nie jest to problem w przypadku pojedynczego użycia algorytmu, bo 100 generacji dla populacji 200 osobników przebiega w około 12 sekund. Zwiększający się czas wykonania może być jednak uciążliwy w przypadku wielokrotnych uruchomień w celu testowania.

Wyniki na dla liczby generacji równej 5 prawie odpowiadają przypadkowi, w którym wybierany jest najlepszy osobnik z losowej populacji. Nie świadczy to dobrze o algorytmie, sugerując że całe jego skomplikowanie jest redundantne. Jednak dla wysokości drzewa ustalonej na 2, optymalnej według poprzednich testów, otrzymane wyniki uzasadniają użycie algorytmu genetycznego i lepiej odpowiadają literaturze \ref{wykresyWielkosc2}. Tym razem, prawdopodobnie ze względu na ograniczenie potencjalnego dopasowanie do danych treningowych, do pewnego stopnia korzystne jest umożliwienie dłuższej pracy algorytmu, jak i dostarczenie większej liczby osobników.

\begin{figure}[h]
\centering
\caption{Działanie algorytmu z różną liczbą generacji i wielkości populacji dla wielkości drzewa = 5}
\label{wykresyWielkosc1}
\begin{tikzpicture}
\begin{groupplot}[ group style={group size= 2 by 2, horizontal sep=3cm,  vertical sep=3cm}, ylabel near ticks, xlabel near ticks, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=5}, width=.45\textwidth]

\nextgroupplot[align=center, title={\scriptsize Różne wielkości populacji \\ \scriptsize na zbiorze treningowym}, ylabel=zysk na zbiorze treningowym, xlabel=wielkość populacji]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {popSize1.csv};
 
\nextgroupplot[align=center, title={\scriptsize Różne wielkości populacji \\ \scriptsize na zbiorze testowym}, ylabel=zysk na zbiorze testowym, xlabel=wielkość populacji]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {popSize1Test.csv};

\nextgroupplot[align=center, title={\scriptsize Różna liczba generacji \\ \scriptsize na zbiorze treningowym}, ylabel=zysk na zbiorze treningowym, xlabel=liczba generacji]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {genCount1.csv};

\nextgroupplot[align=center, title={\scriptsize Różna liczba generacji \\ \scriptsize na zbiorze testowym}, ylabel=zysk na zbiorze testowym, xlabel=liczba generacji]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {genCount1Test.csv};
\end{groupplot}
\end{tikzpicture}
\end{figure}
...
\begin{figure}[h]
\centering
\caption{Działanie algorytmu z różną liczbą generacji i wielkości populacji dla wielkości drzewa = 2}
\label{wykresyWielkosc2}
\begin{tikzpicture}
\begin{groupplot}[ group style={group size= 2 by 2, horizontal sep=3cm,  vertical sep=3cm}, ylabel near ticks, xlabel near ticks, xticklabel style={/pgf/number format/fixed, /pgf/number format/precision=5}, width=.45\textwidth]

\nextgroupplot[align=center, title={\scriptsize Różne wielkości populacji \\ \scriptsize na zbiorze treningowym}, ylabel=zysk na zbiorze treningowym, xlabel=wielkość populacji]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {popSize2.csv};
 
\nextgroupplot[align=center, title={\scriptsize Różne wielkości populacji \\ \scriptsize na zbiorze testowym}, ylabel=zysk na zbiorze testowym, xlabel=wielkość populacji]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {popSize2Test.csv};

\nextgroupplot[align=center, title={\scriptsize Różna liczba generacji \\ \scriptsize na zbiorze treningowym}, ylabel=zysk na zbiorze treningowym, xlabel=liczba generacji]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {genCount2.csv};

\nextgroupplot[align=center, title={\scriptsize Różna liczba generacji \\ \scriptsize na zbiorze testowym}, ylabel=zysk na zbiorze testowym, xlabel=liczba generacji]
\addplot 
table [col sep = semicolon, x index=0, y index=1] {genCount2Test.csv};
\end{groupplot}
\end{tikzpicture}
\end{figure}

\section {Wyniki po dostosowaniu hiperparametrów}

\begin{table}[h]
\pgfplotstabletypeset[
    col sep=semicolon,
    string type,
    columns/0/.style={column name=Name, column type={|l}},
